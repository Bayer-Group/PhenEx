...
async def create_constant(request: Request, study_id: str, data: ConstantCreate):
    ...
    valid_types = ['RelativeTimeRangeFilter', 'CategoricalFilter', 'DateFilter', 'array']
    if data.type not in valid_types:
        raise HTTPException(status_code=400, detail=f"Invalid constant type. Must be one of: {', '.join(valid_types)}")
    ...
    try:
        ...
        if "unique constraint" in error_msg.lower():
            raise HTTPException(status_code=409, detail=f"Constant with name '{data.name}' already exists in this study")
...
async def update_constant(request: Request, study_id: str, constant_id: str, data: ConstantUpdate):
    """
