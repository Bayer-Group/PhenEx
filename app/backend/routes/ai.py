...
try:
    ...
    from ..atomic_functions import (
        update_value_filter, update_relative_time_range, update_codelist,
        update_name, update_description, update_domain, update_type, 
        update_return_date, update_categorical_filter, update_nested_phenotype,
        update_component_date_select,
        get_phenotype_capabilities, PhenotypeValidationError
    )
except ImportError:
    # Fallback for direct imports (for testing)
    from atomic_functions import (
...
@agent.tool  
async def create_phenotype(ctx: RunContext[CohortContext], call: CreatePhenotypeCall) -> str:
    ...
    try:
        ...
        logger.info(f"🔧 TOOL CALL: create_phenotype(name='{call.name}', class_name='{call.class_name}', type='{call.type}', description='{call.description}')")
        
        streaming_ctx = get_streaming_context()
        if streaming_ctx:
            streaming_ctx.stream_message("tool_call", f"➕ Creating phenotype: {call.name}")
        ...
        if call.type == "entry":
            existing_entry = [p for p in existing_phenotypes if p.get("type") == "entry"]
            if existing_entry:
        ...
        if call.class_name in ["CodelistPhenotype", "MeasurementPhenotype"]:
            ...
            logger.info(f"🔧 AUTO-INITIALIZED return_date='first' for {call.class_name}")
        ...
        if call.class_name == "EventCountPhenotype":
            ...
            logger.info(f"🔧 AUTO-INITIALIZED component_date_select='second' and return_date='last' for EventCountPhenotype")
        ...
        if streaming_ctx:
            streaming_ctx.stream_message("tool_error", f"❌ Error creating {call.name}: {str(e)}")
        return f"❌ Error creating {call.name}: {str(e)}"

@agent.tool
async def delete_phenotype(ctx: RunContext[CohortContext], call: DeletePhenotypeCall) -> str:
    ...
    try:
        ...
        # Log detailed tool call with parameters
        logger.info(f"🔧 TOOL CALL: delete_phenotype(id='{call.id}', explanation='{call.explanation}')")
        ...
        if streaming_ctx:
            streaming_ctx.stream_message("tool_result", f"✅ Successfully deleted {phenotype_name}")
        ...
        streaming_ctx = get_streaming_context()
        if streaming_ctx:
            streaming_ctx.stream_message("tool_error", f"Error deleting {call.id}: {str(e)}")
        return f"❌ Error deleting {call.id}: {str(e)}"

# ================== ATOMIC UPDATE TOOLS ==================

@agent.tool
async def atomic_update_value_filter(ctx: RunContext[CohortContext], phenotype_id: str, value_filter: Optional[Dict]) -> str:
    ...
    print(f"{'='*80}\n")
    logger.info(f"🔧 ATOMIC_UPDATE_VALUE_FILTER: phenotype_id='{phenotype_id}', value_filter_type={type(value_filter)}, value_filter={value_filter}")
    ...
    # Log detailed tool call with parameters
    logger.info(f"🔧 TOOL CALL: atomic_update_value_filter(phenotype_id='{phenotype_id}', value_filter={value_filter})")
    
    # Send streaming feedback
    streaming_ctx = get_streaming_context()
    if streaming_ctx:
        streaming_ctx.stream_message("tool_call", f"🔢 Updating value filter for {phenotype_name}")
    
    # Handle removal case
    if value_filter is None:
        # Remove value filter entirely
        logger.info(f"🔧 REMOVING value filter for phenotype {phenotype_id} ({phenotype_name})")
        result = await update_value_filter(ctx, phenotype_id, None)
    else:
        # Validate structure for non-None case
        if not isinstance(value_filter, dict) or value_filter.get("class_name") != "ValueFilter":
        ...
        logger.info(f"🔧 SETTING value filter for phenotype {phenotype_id} ({phenotype_name}) with cleaned params: {cleaned_params}")
        result = await update_value_filter(ctx, phenotype_id, cleaned_params)
    
    if streaming_ctx:
        streaming_ctx.stream_message("tool_result", f"✅ Successfully updated value filter for {phenotype_name}")
    
    # Automatically inject cohort state verification
    state_check = await auto_inject_cohort_state(ctx, f"updating value filter for {phenotype_id}")
    
    return result + state_check

@agent.tool  
async def atomic_update_relative_time_range(ctx: RunContext[CohortContext], phenotype_id: str, relative_time_range: Optional[Union[Dict, List[Dict]]]) -> str:
    ...
    # Log detailed tool call with parameters
    logger.info(f"🔧 TOOL CALL: atomic_update_relative_time_range(phenotype_id='{phenotype_id}', phenotype_class='{phenotype_class}', relative_time_range={relative_time_range})")
    ...
    if relative_time_range is not None:
        ...
        if phenotype_class in ["TimeRangePhenotype", "DeathPhenotype"]:
            ...
            if isinstance(relative_time_range, dict):
                ...
                logger.info(f"🔄 CONVERTED single object to array for {phenotype_class}: {relative_time_range}")
    ...
    streaming_ctx = get_streaming_context()
    if streaming_ctx:
        streaming_ctx.stream_message("tool_call", f"📅 Updating time range for {phenotype_name}")
    
    result = await update_relative_time_range(ctx, phenotype_id, relative_time_range)
    
    if streaming_ctx:
        streaming_ctx.stream_message("tool_result", f"✅ Successfully updated time range for {phenotype_name}")
    
    # Automatically inject cohort state verification
    state_check = await auto_inject_cohort_state(ctx, f"updating time range for {phenotype_id}")
...
async def atomic_update_codelist(
    ...
    print(f"manual_codelist: {json.dumps(manual_codelist, indent=2) if manual_codelist else None}")
    print(f"{'='*80}\n")
    
    # Validate parameters - only ONE mode should be provided
    modes_provided = sum([
        codelist_name is not None,
        manual_codelist is not None
    ])
        ...
        try:
            ...
            try:
                ...
                if codelist_id:
                    ...
                    """
                    all_files = await conn.fetch(sql_query, user_id, cohort_id, codelist_id)
                else:
            ...
            for file_row in all_files:
                ...
                if not codelist_col:
                    print(f"  ⚠️  File {file_row['file_id']}: No codelist column found in columns: {list(data.keys())}")
                    continue
                
                print(f"  ✓ File {file_row['file_id']}: Using codelist column '{codelist_col}'")
                ...
                print(f"  📊 Found {len(unique_codelists)} unique codelists: {sorted(list(unique_codelists))}")
                ...
                for cl_name in unique_codelists:
                    if cl_name.lower() == codelist_name_lower:
                        print(f"  ✅ CASE-INSENSITIVE MATCH: '{codelist_name}' → '{cl_name}'")
                        result = file_row
                ...
                print(f"  🔄 Trying fuzzy match (space→underscore): '{codelist_name}' → '{codelist_name_alt}'")
                for cl_name in unique_codelists:
                    if cl_name.lower() == codelist_name_alt.lower():
                        print(f"  ✅ FUZZY MATCH (space→_): '{codelist_name}' → '{cl_name}'")
                        result = file_row
                        found_codelist_name = cl_name
                        break
                ...
                print(f"  🔄 Trying fuzzy match (_→space): '{codelist_name}' → '{codelist_name_alt2}'")
                for cl_name in unique_codelists:
                    if cl_name.lower() == codelist_name_alt2.lower():
                        print(f"  ✅ FUZZY MATCH (_→space): '{codelist_name}' → '{cl_name}'")
                        result = file_row
                        found_codelist_name = cl_name
                        break
            ...
            if not result or not found_codelist_name:
                print(f"❌ LOOKUP FAILED: Codelist '{codelist_name}' not found in any file")
            ...
            if not file_id_found:
                print(f"❌ INVALID FILE ID: Codelist '{codelist_name}' was found but has no file ID")
            ...
            codelist_to_set = {
                ...
                "codelist": {
                    ...
                    "file_name": result.get('codelists', 'unknown_file'),  # Get filename from database
                    "codelist_name": found_codelist_name,  # Specific codelist name within the file
                    "code_column": column_mapping.get("code_column", "code"),
                    "code_type_column": column_mapping.get("code_type_column", "code_type"),
                    "codelist_column": column_mapping.get("codelist_column", "codelist")
                },
            ...
            print(f"   code_type_column: {column_mapping.get('code_type_column', 'code_type')}")
            print(f"   codelist_column: {column_mapping.get('codelist_column', 'codelist')}")
            print(f"   codelist_type: from file")
            logger.info(f"✅ Created reference to codelist '{found_codelist_name}' (user requested: '{codelist_name}') from file '{file_id_found}'")
    ...
    # CASE 3: Manual codelist
        ...
        logger.info(f"✅ Created manual codelist with {sum(len(v) for v in manual_codelist['codelist'].values())} codes")
    ...
    # Log detailed tool call with parameters  
    logger.info(f"🔧 TOOL CALL: atomic_update_codelist(phenotype_id='{phenotype_id}', codelist_name={codelist_name}, codelist_id={codelist_id}, manual_codelist={bool(manual_codelist)})")
    
    # Send streaming feedback
    streaming_ctx = get_streaming_context()
    if streaming_ctx:
        if phenotype_name:
            streaming_ctx.stream_message("tool_call", f"📋 Updating codelist for {phenotype_name}")
        else:
            ...
            streaming_ctx.stream_message("tool_error", f"❌ Could not find phenotype. Available phenotypes: {', '.join(valid_names)}")
    
    if codelist_to_set is None:
        logger.info(f"🔧 REMOVING codelists for phenotype {phenotype_id} ({phenotype_name})")
    else:
        logger.info(f"🔧 SETTING codelists for phenotype {phenotype_id} ({phenotype_name}) with params: {codelist_to_set}")
        ...
        if phenotype:
            ...
            if phenotype_class in ["CodelistPhenotype", "MeasurementPhenotype"]:
                ...
                if current_return_date is None:
                    logger.info(f"🔧 AUTO-SETTING return_date='first' for {phenotype_class} (was None)")
                    await update_return_date(ctx, phenotype_id, "first")
                    result += f"\n🔧 Automatically set return_date='first' (required for {phenotype_class})"
    
    if streaming_ctx:
        if result.startswith("✅"):
            streaming_ctx.stream_message("tool_result", f"✅ Successfully updated codelist for {phenotype_name}")
        else:
            streaming_ctx.stream_message("tool_error", f"Failed to update codelist for {phenotype_name}")
    
    # Automatically inject cohort state verification (only if successful)
    if result.startswith("✅"):
        state_check = await auto_inject_cohort_state(ctx, f"updating codelist for {phenotype_id}")
        return result + state_check
    else:
        return result

@agent.tool
async def atomic_update_name(ctx: RunContext[CohortContext], phenotype_id: str, name: str) -> str:
    """Update name of a phenotype."""
    # Log detailed tool call with parameters
    logger.info(f"🔧 TOOL CALL: atomic_update_name(phenotype_id='{phenotype_id}', name='{name}')")
    
    # Send streaming feedback
    streaming_ctx = get_streaming_context()
    if streaming_ctx:
        streaming_ctx.stream_message("tool_call", f"🏷️ Updating phenotype name to '{name}'")
    
    result = await update_name(ctx, phenotype_id, name)
    
    if streaming_ctx:
        streaming_ctx.stream_message("tool_result", f"✅ Successfully updated phenotype name")
    
    return result

@agent.tool
async def atomic_update_description(ctx: RunContext[CohortContext], phenotype_id: str, description: str) -> str:
    """Update description of a phenotype."""
    # Log detailed tool call with parameters
    logger.info(f"🔧 TOOL CALL: atomic_update_description(phenotype_id='{phenotype_id}', description='{description}')")
    return await update_description(ctx, phenotype_id, description)

@agent.tool  
async def atomic_update_domain(ctx: RunContext[CohortContext], phenotype_id: str, domain: str) -> str:
    """Update domain of a phenotype with validation for allowed domains."""
    return await update_domain(ctx, phenotype_id, domain)

@agent.tool
async def atomic_update_type(ctx: RunContext[CohortContext], phenotype_id: str, phenotype_type: str) -> str:
    """Update type (inclusion/exclusion/etc) of a phenotype."""
    return await update_type(ctx, phenotype_id, phenotype_type)

@agent.tool
async def atomic_update_return_date(ctx: RunContext[CohortContext], phenotype_id: str, return_date: str) -> str:
    ...
    """
    # Log detailed tool call with parameters
    logger.info(f"🔧 TOOL CALL: atomic_update_return_date(phenotype_id='{phenotype_id}', return_date='{return_date}')")
    
    # Validate return_date value
    valid_values = ["first", "last", "nearest", "all"]
    if return_date not in valid_values:
        return f"❌ ERROR: return_date must be one of {valid_values}, got: {return_date}"
    
    return await update_return_date(ctx, phenotype_id, return_date)

@agent.tool
async def atomic_update_categorical_filter(ctx: RunContext[CohortContext], phenotype_id: str, categorical_filter: Optional[Dict]) -> str:
    ...
    """
    # Log detailed tool call with parameters
    logger.info(f"🔧 TOOL CALL: atomic_update_categorical_filter(phenotype_id='{phenotype_id}', categorical_filter={categorical_filter})")
    return await update_categorical_filter(ctx, phenotype_id, categorical_filter)

@agent.tool
async def atomic_update_nested_phenotype(ctx: RunContext[CohortContext], phenotype_id: str, nested_phenotype: Dict) -> str:
    ...
    logger.info(f"🔧 TOOL CALL: atomic_update_nested_phenotype(phenotype_id='{phenotype_id}', nested_phenotype={nested_phenotype})")
    
    # Validate that nested phenotype has return_date="all" for EventCountPhenotype
    if nested_phenotype.get("return_date") != "all":
        logger.warning(f"⚠️  Nested phenotype should have return_date='all' for EventCountPhenotype, got: {nested_phenotype.get('return_date')}")
        nested_phenotype["return_date"] = "all"
        logger.info(f"🔧 AUTO-CORRECTED return_date to 'all' for EventCountPhenotype compatibility")
    ...
    streaming_ctx = get_streaming_context()
    if streaming_ctx:
        streaming_ctx.stream_message("tool_call", f"🔗 Updating nested phenotype for {phenotype_name}")
    
    result = await update_nested_phenotype(ctx, phenotype_id, nested_phenotype)
    
    if streaming_ctx:
        if result.startswith("✅"):
            streaming_ctx.stream_message("tool_result", f"✅ Successfully updated nested phenotype for {phenotype_name}")
        else:
            streaming_ctx.stream_message("tool_error", f"Failed to update nested phenotype for {phenotype_name}")
    
    # Automatically inject cohort state verification (only if successful)
    if result.startswith("✅"):
        state_check = await auto_inject_cohort_state(ctx, f"updating nested phenotype for {phenotype_id}")
        return result + state_check
    else:
        return result

@agent.tool
async def atomic_update_component_date_select(ctx: RunContext[CohortContext], phenotype_id: str, component_date_select: str) -> str:
    ...
    """
    # Log detailed tool call
    logger.info(f"🔧 TOOL CALL: atomic_update_component_date_select(phenotype_id='{phenotype_id}', component_date_select='{component_date_select}')")
    ...
    streaming_ctx = get_streaming_context()
    if streaming_ctx:
        streaming_ctx.stream_message("tool_call", f"📅 Setting component_date_select={component_date_select} for {phenotype_name}")
    
    result = await update_component_date_select(ctx, phenotype_id, component_date_select)
    
    if streaming_ctx:
        if result.startswith("✅"):
            streaming_ctx.stream_message("tool_result", f"✅ Successfully set component_date_select for {phenotype_name}")
        else:
            streaming_ctx.stream_message("tool_error", f"Failed to set component_date_select for {phenotype_name}")
    
    # Automatically inject cohort state verification (only if successful)
    if result.startswith("✅"):
        state_check = await auto_inject_cohort_state(ctx, f"updating component_date_select for {phenotype_id}")
        return result + state_check
    else:
        return result
...
async def list_codelists(ctx: RunContext[CohortContext]) -> str:
    ...
    **Example user phrases that should trigger this tool:**
    ...
    streaming_ctx = get_streaming_context()
    if streaming_ctx:
        streaming_ctx.stream_message("tool_call", "� Listing all available codelists...")
    ...
    try:
        ...
        result_msg = f"✅ Found {len(results)} codelists across {len(all_codelists)} file(s):\n\n" + "\n".join(results)
...
async def auto_inject_cohort_state(ctx: RunContext[CohortContext], operation_description: str) -> str:
    ...
    streaming_ctx = get_streaming_context()
    if streaming_ctx:
        streaming_ctx.stream_message("tool_call", f"🔍 Auto-checking cohort state after {operation_description}")
...
async def update_context_only(context: CohortContext, updated_phenotypes: List[Dict], change_description: str):
    ...
    async with get_context_lock():
        ...
        if 'phenotypes' in context.current_cohort:
            ...
            print(f"🔄 UPDATE_CONTEXT: ✅ Updated context phenotypes to phenotype IDs: {[p.get('id') for p in updated_phenotypes]}")
        else:
            print(f"🔄 UPDATE_CONTEXT: Warning - no 'phenotypes' key in context.current_cohort")
...
async def save_updated_cohort(context: CohortContext, updated_phenotypes: List[Dict], change_description: str):
    ...
    async with get_save_lock():
        try:
            ...
            print(f"💾 SAVE_COHORT: Current_cohort object ID: {id(context.current_cohort)}")
            ...
            for category in ["entry_criterion", "inclusions", "exclusions", "characteristics", "outcomes"]:
                ...
                if isinstance(cat_data, list):
                    ...
                    for i, item in enumerate(cat_data):
                        print(f"💾 SAVE_COHORT:     {i}: {item.get('name')} (ID: {item.get('id')})")
            
            # Save as provisional, replacing existing provisional version
            print(f"💾 SAVE_COHORT: Saving to database...")
            logger.info(f"Saving provisional changes: user_id={context.user_id}, cohort_id={context.cohort_id}, study_id={context.study_id}")
            ...
            if "phenotypes" in context.current_cohort:
                print(f"💾 SAVE_COHORT:   Context has 'phenotypes' field with {len(context.current_cohort['phenotypes'])} items")
            for category in ["inclusions", "exclusions"]:
                cat_data = context.current_cohort.get(category, [])
                if isinstance(cat_data, list):
                    print(f"💾 SAVE_COHORT:   Context {category}: {len(cat_data)} items")
                    for item in cat_data:
                        print(f"💾 SAVE_COHORT:     - {item.get('name')} (ID: {item.get('id')})")
...
async def suggest_changes_v2(
    ...
    try:
        for field in ["entry_criterion", "inclusions", "exclusions", "characteristics", "outcomes"]:
    ...
    async def stream_ai_response():
        ...
        try:
            ...
            if actual_tool_calls:
                print(f"💾 FINAL_SAVE: ✅ Tools were executed ({len(actual_tool_calls)} calls) - assuming changes were made")
                content_changed = True
            elif final_ids == initial_ids and len(final_phenotypes) == len(initial_phenotypes):
                # Only do expensive deep comparison if no tools were called
                print(f"💾 FINAL_SAVE: No tools called - doing deep content comparison for {len(final_ids)} phenotypes")
                for pid in final_ids:
                    initial_pheno = initial_lookup[pid]
                    final_pheno = final_lookup[pid]
                    print(f"💾 FINAL_SAVE: Comparing phenotype {pid}:")
                    print(f"💾 FINAL_SAVE:   Initial: {json.dumps(initial_pheno, sort_keys=True)[:200]}...")
                    print(f"💾 FINAL_SAVE:   Final:   {json.dumps(final_pheno, sort_keys=True)[:200]}...")
                ...
                if not content_changed:
                    print(f"💾 FINAL_SAVE: ❌ No content changes detected in any phenotype")
            else:
                # Different phenotype sets detected
                content_changed = True
                print(f"💾 FINAL_SAVE: ✅ Structural changes detected (different phenotype sets)")
            
            if final_ids != initial_ids or len(final_phenotypes) != len(initial_phenotypes) or content_changed:
                print(f"\n💾 FINAL_SAVE: Changes detected - saving final state")
                print(f"💾 FINAL_SAVE: Initial: {len(initial_phenotypes)} phenotypes {list(initial_ids)}")
                print(f"💾 FINAL_SAVE: Final: {len(final_phenotypes)} phenotypes {list(final_ids)}")
                if content_changed:
                    print(f"💾 FINAL_SAVE: Content changes detected in existing phenotypes")
                ...
                await save_final_cohort(context, "Final save after AI conversation completed")
            ...
            if tool_calls_made:
                logger.info(f"🔧 AI TOOL CALL SUMMARY ({len(tool_calls_made)} operations):")
