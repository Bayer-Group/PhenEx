...
class MockDB:
    ...
    async def update_cohort_for_user(self, user_id, cohort_id, cohort_data, study_id, provisional=True, new_version=False):
    ...
    async def execute(self, query: str, params: tuple = ()):
        ...
        if "FROM codelists" in query and ("LIKE" in query or "=" in query):
            ...
            if "LIKE" in query:
                ...
                search_term = params[1].replace('%', '').lower() if len(params) > 1 else ""
                results = []
                for name, codelist_data in MOCK_CODELISTS.items():
                    if search_term in name.lower() or search_term in codelist_data.get("description", "").lower():
                        results.append((
                            codelist_data["id"],
                            codelist_data["name"],
...
MOCK_CODELISTS = {
    ...
    "Type 2 Diabetes": {
        ...
        "codes": {
            "ICD10": ["E11", "E11.0", "E11.1", "E11.2", "E11.9"]
        }
    },
    "Sepsis and Septic Shock": {
        ...
        "codes": {
            "ICD10": ["A41.9", "R65.20", "R65.21", "A41.0", "A41.1"]
        }
    },
    "Heart Failure": {
        "id": "cl_hf_001",
        ...
        "codes": {
            "ICD10": ["I50", "I50.0", "I50.1", "I50.9", "I11.0"]
        }
    },
    "HbA1c LOINC": {
        "id": "cl_hba1c_001",
...
async def mock_codelist_search(query: str) -> List[Dict]:
    ...
    matches = [
        cl for name, cl in MOCK_CODELISTS.items()
        if query_lower in name.lower() or query_lower in cl.get("description", "").lower()
    ]
    return matches
...
class MockCohortUtils:
    def convert_phenotypes_to_structure(self, cohort_data):
        print(f"🔄 CohortUtils.convert_phenotypes_to_structure called with {len(cohort_data.get('phenotypes', []))} phenotypes")
        return cohort_data
...
mock_rag.query_faiss_index = lambda **x: ["Documentation about PhenEx phenotypes and parameters"]
...
def print_phenotypes(phenotypes, title="Phenotypes"):
    ...
    for i, phenotype in enumerate(phenotypes, 1):
        ...
        if phenotype.get('value_filter'):
            ...
            min_val = vf.get('min_value', {}).get('value') if vf.get('min_value') else None
            max_val = vf.get('max_value', {}).get('value') if vf.get('max_value') else None
...
def compare_cohorts(initial_cohort, final_cohort):
    ...
    if added_ids:
        ...
        for phenotype in final_phenotypes:
            if phenotype.get('id') in added_ids:
                print(f"   + {phenotype.get('id')}: {phenotype.get('name')} ({phenotype.get('class_name')})")
                print(f"     RAW JSON:")
    ...
    for phenotype_id in common_ids:
        initial_p = next((p for p in initial_phenotypes if p.get('id') == phenotype_id), None)
        final_p = next((p for p in final_phenotypes if p.get('id') == phenotype_id), None)
        
        if initial_p and final_p:
            # Compare key fields
            changes = []
            if initial_p.get('name') != final_p.get('name'):
                changes.append(f"name: '{initial_p.get('name')}' → '{final_p.get('name')}'")
            ...
            if initial_p.get('type') != final_p.get('type'):
                changes.append(f"type: '{initial_p.get('type')}' → '{final_p.get('type')}'")
...
def validate_cohort_with_phenex(cohort_data):
    ...
    try:
        ...
        for i, phenotype_data in enumerate(phenotypes, 1):
            ...
            try:
                print(f"   {i}. Testing {phenotype_id} - '{phenotype_name}' ({class_name})...")
                
                # Use standalone from_dict function for all phenotypes
                phenotype = from_dict(phenotype_data.copy())  # Use copy to avoid modifying original
...
def automated_validation(scenario, initial_cohort, final_cohort, ai_actions) -> dict:
    ...
    def add_check(name: str, expected, actual, passed: bool = None, critical: bool = True):
        ...
        if passed:
            ...
            if critical:
                ...
                validation_results['errors'].append(f"{name}:\nExpected:\n\t{expected}\nGot:\n\nt{actual}")
            else:
                validation_results['warnings'].append(f"{name}:\nExpected:\n\t{expected}\nGot:\n\t{actual}")
            ...
            if isinstance(expected_call, str):
                ...
                if not was_called and tool_name == 'add_phenotype':
                    ...
                    if 'create_phenotype' in actual_tool_calls:
                        was_called = True
                        print(f"      ℹ️  Note: 'create_phenotype' + atomic functions used instead of 'add_phenotype' (acceptable)")
                ...
                matching_calls = [action for action in ai_actions if action['tool'] == tool_name]
                ...
                if expected_params:
                    ...
                    param_summary = ', '.join([f"{k}={v}" for k, v in expected_params.items()])
                    add_check(f"Tool '{tool_name}' called with correct params ({param_summary})", 
                             True, found_match)
                else:
    ...
    if scenario.unexpected_tool_calls:
        print(f"   Unexpected tool calls (should NOT be called): {scenario.unexpected_tool_calls}")
        ...
        for unexpected_tool in scenario.unexpected_tool_calls:
            ...
            add_check(f"Tool '{unexpected_tool}' was NOT called", 
                     False, was_called,
                     passed=(not was_called))
            if was_called:
                print(f"      ⚠️  Tool '{unexpected_tool}' was called {call_count} times")
    ...
    if scenario.expected_final_cohort is not None:
        ...
        if wildcard_count > 0:
            ...
            add_check("Final phenotype count", len(expected_phenotypes), len(actual_phenotypes))
            missing_ids = expected_ids - actual_ids
            if missing_ids:
                add_check("Expected phenotype IDs present", f"Missing: {missing_ids}", "All present", 
                         passed=False, critical=False)
            else:
                add_check("Expected phenotype IDs present", "All present", "All present", 
        ...
        for expected_p in expected_phenotypes:
            ...
            if p_id == '*':
                ...
                for actual_p in actual_phenotypes:
                    ...
                    if (expected_class == '*' or actual_p.get('class_name') == expected_class) and \
                       (expected_type == '*' or actual_p.get('type') == expected_type):
                        matched_actual = actual_p
                        break
                
                if not matched_actual:
                    add_check(f"Wildcard phenotype (class={expected_class}, type={expected_type}) found", 
                             True, False, critical=False)
                    continue
                ...
                actual_p = next((p for p in actual_phenotypes if p.get('id') == p_id), None)
                p_display_id = p_id
            ...
            if actual_p:
                ...
                for field in ['name', 'class_name', 'type', 'description', 'domain']:
                    if field in expected_p:
                        ...
                        add_check(f"Phenotype {p_display_id} '{field}'", expected_val, actual_val, 
                                 critical=False)
                
                # Compare complex fields (supporting nested wildcards)
                for field in ['codelist', 'value_filter', 'relative_time_range', 'categorical_filter']:
                    if field in expected_p:
                        expected_val = expected_p.get(field)
                        actual_val = actual_p.get(field)
                        ...
                        if not match:
                            ...
                            print(f"         Expected: {json.dumps(expected_val, indent=10)}")
                            print(f"         Actual:   {json.dumps(actual_val, indent=10)}")
                        
                        # Pass the actual structures to add_check, not just "matches"/"differs"
                        add_check(f"Phenotype {p_display_id} '{field}' structure matches", 
                                 expected_val,
                                 actual_val,
                                 passed=match, critical=False)
    else:
    ...
    if ai_actions:
        ...
        if "deletion" in scenario.name.lower() or "remove" in scenario.user_request.lower():
            has_delete_tool = 'delete_phenotype' in tool_names
            add_check("Used delete_phenotype for removal", True, has_delete_tool, critical=False)
        
        if "add" in scenario.user_request.lower() or "create" in scenario.user_request.lower():
            has_creation_tool = any(tool in tool_names for tool in ['add_phenotype', 'create_phenotype'])
            add_check("Used creation tool for adding", True, has_creation_tool, critical=False)
    ...
    if PHENEX_AVAILABLE:
        ...
        add_check("PhenEx Compatibility", "Skipped (imports unavailable)", "Skipped (imports unavailable)", passed=True, critical=False)
    
    # Calculate final score
    if validation_results['total_checks'] > 0:
        score_percentage = (validation_results['score'] / validation_results['total_checks']) * 100
...
class TestScenario:
    ...
    def __init__(self, name, initial_cohort, user_request, expected_description, 
                 expected_final_cohort=None, expected_tool_calls=None, unexpected_tool_calls=None):
        self.name = name
        ...
        self.expected_tool_calls = expected_tool_calls or []  # List of tool names or detailed specs
        self.unexpected_tool_calls = unexpected_tool_calls or []  # Tools that should NOT be called
    ...
    return {
        ...
        "phenotypes": [
            ...
            },
            {
                ...
                "value_filter": {
                    "class_name": "ValueFilter",
                        "min_value": {"class_name": "GreaterThan", "value": 6.5, "operator": ">"}
                }
            }
        ]
    }
...
def define_test_scenarios():
    ...
    def create_expected_cohort_with_phenotypes(phenotype_ids):
        ...
        expected_phenotypes = [p for p in base_phenotypes if p.get('id') in phenotype_ids]
    ...
    scenarios = [
        ...
        TestScenario(
            ...
            unexpected_tool_calls=['add_phenotype', 'create_phenotype', 'delete_phenotype', 'atomic_update_value_filter', 
                                   'atomic_update_description', 'atomic_update_codelist', 'atomic_update_relative_time_range'],
        ...
        TestScenario(
            ...
            unexpected_tool_calls=['add_phenotype', 'create_phenotype', 'update_phenotype'],
            expected_final_cohort=create_expected_cohort_with_phenotypes(['age_001', 'diabetes_001'])
        ),
        ...
        TestScenario(
            ...
            expected_tool_calls=[
                {'tool': 'atomic_update_value_filter', 'params': {'phenotype_id': 'age_001'}},
            ],
            unexpected_tool_calls=['add_phenotype', 'create_phenotype', 'delete_phenotype'],
            expected_final_cohort={
                "id": "test_cohort_001",
                "name": "Diabetes Research Cohort",
                "class_name": "Cohort",
                "phenotypes": [
                    # Updated age phenotype with new range
                    {
                        ...
                        "value_filter": {
                            "class_name": "ValueFilter",
                            "min_value": {"class_name": "GreaterThanOrEqualTo", "value": 25, "operator": ">="},
                            "max_value": {"class_name": "LessThanOrEqualTo", "value": 70, "operator": "<="}
                        }
                    },
        ...
        TestScenario(
            ...
            expected_tool_calls=[
                {'tool': 'atomic_update_relative_time_range', 'params': {'phenotype_id': 'diabetes_001'}},
            ],
            unexpected_tool_calls=['add_phenotype', 'create_phenotype', 'delete_phenotype'],
            expected_final_cohort={
                "id": "test_cohort_001",
                ...
                "phenotypes": [
                    ...
                    *[p for p in create_base_cohort()['phenotypes']],
                    # First new phenotype - Pregnancy
                    {
                        ...
                        "domain": "CONDITION_OCCURRENCE",
                        "codelist": {
                            "codelist": {"ICD10": "*"},  # Should contain Z33, Z34, O00-O99 but format may vary
                            "class_name": "Codelist",
                            "codelist_type": "manual",
                            "use_code_type": True,
                            "remove_punctuation": False
                    ...
                    # Second new phenotype - Cancer
                    {
                        ...
                        "domain": "CONDITION_OCCURRENCE",
                        "codelist": {
                            "codelist": {"ICD10": "*"},  # Should contain C00-C97 but format may vary
                            "class_name": "Codelist",
                            "codelist_type": "manual",
                            "use_code_type": True,
                            "remove_punctuation": False
        ...
        TestScenario(
            ...
            expected_final_cohort={
                ...
                "phenotypes": [
                    # Age phenotype unchanged
                    *[p for p in create_base_cohort()['phenotypes'] if p['id'] == 'age_001'],
                    ...
                    *[p for p in create_base_cohort()['phenotypes'] if p['id'] == 'hba1c_001'],
                ]
            }
        ),
...
async def run_test_scenario(scenario: TestScenario, scenario_num: int):
    ...
    print_phenotypes(scenario.initial_cohort.get('phenotypes', []), "Initial Phenotypes")
...
"""        
    ...
    validation_results = automated_validation(scenario, scenario.initial_cohort, final_cohort, ai_actions)
    ...
    if overall_success:
        ...
        print(f"   ✅ Expected outcomes achieved ({validation_results['score_percentage']:.1f}% score)")
        print(f"   ℹ️  Phenotype count change: {count_change:+d} (from {initial_count} to {final_count})")
    else:
        ...
        if ai_error:
            ...
            print(f"   ❌ AI made tool calls ({len(ai_actions)}) but should have made none (information-only request)")
            ...
            print(f"   ❌ Expected outcomes not achieved ({validation_results['score_percentage']:.1f}% score)")
        print(f"   ℹ️  Phenotype count change: {count_change:+d} (from {initial_count} to {final_count})")
...
async def run_manual_test_suite(selected_scenarios=None):
    ...
    if selected_scenarios:
        scenarios = [(i, all_scenarios[i-1]) for i in selected_scenarios if 1 <= i <= len(all_scenarios)]
        if len(scenarios) != len(selected_scenarios):
            print(f"⚠️  Warning: Some scenario numbers were out of range (1-{len(all_scenarios)})")
        print(f"🎯 Running {len(scenarios)} selected scenario(s) out of {len(all_scenarios)} total")
    else:
    ...
    for result in results:
        ...
        validation_score = result.get('validation_results', {}).get('score_percentage', 0)
    ...
    print(f"   Successfully Executed: {executed}/{total_scenarios} ({(executed/total_scenarios)*100:.1f}%)")
    print(f"   Automated Validation Passed: {automated_passed}/{total_scenarios} ({(automated_passed/total_scenarios)*100:.1f}%)")
...
def generate_automated_report(results, output_file="ai_validation_report.json"):
    ...
    if report['total_scenarios'] > 0:
        report['summary']['execution_rate'] = (report['summary']['executed'] / report['total_scenarios']) * 100
        report['summary']['pass_rate'] = (report['summary']['automated_passed'] / report['total_scenarios']) * 100
...
if __name__ == "__main__":
    ...
    try:
        if args.automated:
            results = asyncio.run(run_automated_test_suite(selected_scenarios=selected_scenarios))
        else:
            results = asyncio.run(run_manual_test_suite(selected_scenarios=selected_scenarios))
        ...
        if all_passed:
            ...
            print(f"⚠️  {passed_count}/{total_count} scenarios passed automated validation")
