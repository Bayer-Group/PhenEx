...
PHENOTYPE_CAPABILITIES = {
    ...
    "CodelistPhenotype": {
        "supported_fields": ["codelist", "relative_time_range", "name", "description", "domain", "type", "return_date"],
        "requires": ["codelist"],
        "domain_restrictions": ["CONDITION_OCCURRENCE", "PROCEDURE_OCCURRENCE", "DRUG_EXPOSURE", "VISIT_OCCURRENCE"]
    },
    "MeasurementPhenotype": {
        "supported_fields": ["codelist", "value_filter", "relative_time_range", "name", "description", "domain", "type", "return_date"],
        "requires": ["codelist"],
        "domain_restrictions": ["MEASUREMENT"]
    },
    "CategoricalPhenotype": {
        "supported_fields": ["categorical_filter", "name", "description", "domain", "type"],
    ...
    "DeathPhenotype": {
        "supported_fields": ["relative_time_range", "name", "description", "domain", "type", "return_date"],
        "requires": [],
        "domain_restrictions": ["DEATH", "PERSON"]
    },
    "TimeRangePhenotype": {
        "supported_fields": ["relative_time_range", "name", "description", "domain", "type"],
        "requires": ["relative_time_range"],
        "domain_restrictions": ["OBSERVATION_PERIOD", "PERSON", "VISIT_OCCURRENCE"]
    },
    "EventCountPhenotype": {
        "supported_fields": ["phenotype", "value_filter", "relative_time_range", "name", "description", "type", "return_date", "component_date_select"],
        "requires": ["phenotype"],
        "domain_restrictions": []  # Uses domain from component phenotype
    }
}

def validate_phenotype_operation(phenotype: Dict[str, Any], field: str, operation_name: str) -> None:
    """
...
def find_phenotype_by_id(cohort: Dict[str, Any], phenotype_id: str) -> Optional[Dict[str, Any]]:
...
def update_phenotype_field(cohort: Dict[str, Any], phenotype_id: str, field: str, value: Any, operation_name: str) -> AtomicUpdateResult:
    """
...
async def update_value_filter(ctx: RunContext, phenotype_id: str, value_filter: Dict[str, Any]) -> str:
    """
...
async def update_relative_time_range(ctx: RunContext, phenotype_id: str, relative_time_range: Dict[str, Any]) -> str:
    """
    ...
    cohort = ctx.deps.current_cohort
    result = update_phenotype_field(
        cohort, phenotype_id, "relative_time_range", relative_time_range, "update_relative_time_range"
    )
...
async def update_codelist(ctx: RunContext, phenotype_id: str, codelist: Dict[str, Any]) -> str:
    """
...
async def update_description(ctx: RunContext, phenotype_id: str, description: str) -> str:
    """
    ...
    """
    cohort = ctx.deps.current_cohort
    result = update_phenotype_field(cohort, phenotype_id, "description", description, "update_description")
...
async def update_domain(ctx: RunContext, phenotype_id: str, domain: str) -> str:
    ...
    if phenotype:
        ...
        if phenotype_class in PHENOTYPE_CAPABILITIES:
            allowed_domains = PHENOTYPE_CAPABILITIES[phenotype_class]["domain_restrictions"]
    ...
    result = update_phenotype_field(cohort, phenotype_id, "domain", domain, "update_domain")
...
async def update_type(ctx: RunContext, phenotype_id: str, phenotype_type: str) -> str:
    ...
    cohort = ctx.deps.current_cohort
    result = update_phenotype_field(cohort, phenotype_id, "type", phenotype_type, "update_type")
...
async def update_return_date(ctx: RunContext, phenotype_id: str, return_date: bool) -> str:
    """
    ...
    """
    cohort = ctx.deps.current_cohort
    result = update_phenotype_field(cohort, phenotype_id, "return_date", return_date, "update_return_date")
    
    if result.success:
        logger.info(f"Updated return_date for phenotype {phenotype_id} to {return_date}")
    else:
        logger.error(f"Failed to update return_date: {result.message}")
    
    return "✅ " + result.message if result.success else "❌ " + result.message

async def update_categorical_filter(ctx: RunContext, phenotype_id: str, categorical_filter: Dict[str, Any]) -> str:
    """
    ...
    cohort = ctx.deps.current_cohort
    result = update_phenotype_field(
        cohort, phenotype_id, "categorical_filter", categorical_filter, "update_categorical_filter"
    )
...
async def update_nested_phenotype(ctx: RunContext, phenotype_id: str, nested_phenotype: Dict[str, Any]) -> str:
    """
    ...
    if "type" not in nested_phenotype:
        ...
        logger.info(f"🔧 Auto-added type='component' to nested phenotype (nested phenotypes are internal building blocks)")
    elif nested_phenotype["type"] != "component":
        logger.warning(f"⚠️  Nested phenotype has type='{nested_phenotype['type']}', but should be 'component'. Auto-correcting...")
        nested_phenotype["type"] = "component"
...
async def update_component_date_select(ctx: RunContext, phenotype_id: str, component_date_select: str) -> str:
    """
    ...
    cohort = ctx.deps.current_cohort
    result = update_phenotype_field(
        cohort, phenotype_id, "component_date_select", component_date_select, "update_component_date_select"
    )
    
    if result.success:
        logger.info(f"Updated component_date_select to '{component_date_select}' for phenotype {phenotype_id}")
    else:
...
def get_phenotype_capabilities(phenotype_class: str) -> Dict[str, Any]:
    ...
    return PHENOTYPE_CAPABILITIES.get(phenotype_class, {
        "supported_fields": [],
        "requires": [],
        "domain_restrictions": []
    })