...
export const AttritionCellRenderer = forwardRef<AttritionCellRendererRef, AttritionCellRendererProps>((
  {
    rows,
    cohortId,
    databaseSize,
  ...
  useEffect(() => {
    ...
    // Scale constants inversely with pan-zoom scale so content stays
    // readable at small zoom levels (e.g. INITIAL_SCALE = 0.1).
    const s = 1 / pzScale;
    const LEFT_WRAPPER_WIDTH = 420 * s;
    const BOX_CENTER_X = LEFT_WRAPPER_WIDTH / 2;
    const BOX_MAX_WIDTH = 400 * s;
    const BOX_MIN_WIDTH = 160 * s;
    const ROW_HEIGHT = 100 * s;
    const ARROW_HEIGHT = 50 * s;
    const EXCLUDED_BOX_WIDTH = 120 * s;
    const EXCLUDED_BOX_OFFSET = 460 * s;
    const SVG_PADDING = 10 * s;
    const TOTAL_MAX_WIDTH = 620 * s;
    const FONT_TITLE = `${14 * s}px`;
    const FONT_DESC = `${12 * s}px`;
    const FONT_SMALL = `${10 * s}px`;
    const STROKE_W = Math.max(1, 1 * s);
    const BORDER_RADIUS = 8 * s;
  ...
  }, [rows, cohortId, onRowClick, pzScale]);
  ...
  return (
    <div style={{ 
      ...
      <svg ref={svgRef} style={{ display: 'block', margin: 0, padding: 0 }} />
    </div>
  );
});
