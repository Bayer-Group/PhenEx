...
export const SingleCodelistEditor: React.FC<SingleCodelistEditorProps> = ({ value, onValueChange, options, className }) => {
  ...
  const renderEditor = () => {
    ...
    switch (selectedEditor) {
      case 'from file':
        return <FileCodelistEditor {...editorProps} onValueChange={(newValue) => handleValueChange(newValue)}/>;
      case 'from medconb':
        return <MedConBCodelistEditor {...editorProps} onValueChange={(newValue) => handleValueChange(newValue)}/>;
      default:
        return <ManualCodelistEditor {...editorProps} onValueChange={(newValue) => handleValueChange(newValue)}/>;
    }
  };
