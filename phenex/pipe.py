...
class PhenexComputeNode:
    ...
    def __init__(
        self, 
        name: str,
        children: List["PhenexComputeNode"] = None
        ):
        self.name = name
    ...
    def _check_children_are_ok(self):
        ...
        if self.name in child_names:
            raise ValueError(f"Duplicate node name found: '{self.name}' is used both for this node and one of its children.")
    ...
    def execute(
        ...
        Parameters:
            tables (Dict[str, PhenexTable], optional): A dictionary mapping table names to PhenexTable objects. Defaults to None.
        ...
        Returns:
            PhenexTable: The resulting phenotype table for this node.
        """
        logger.info(f"Node '{self.name}': executing ...")
        for child in self.children:
            logger.info(f"Node '{self.name}': executing children nodes ...")
            child.execute(tables = tables, con = con, overwrite = overwrite, lazy_execution = lazy_execution)
            
        if lazy_execution:
            if not overwrite:
                raise ValueError("lazy_execution only works with overwrite=True.")
            if con is None:
                raise ValueError("A DatabseConnector is required for lazy execution. Comupted tables will be materialized and only recomputed as needed.")
    ...
    def _execute(self, tables: Dict[str, PhenexTable]) -> PhenexTable:
        """
        Implements the processing logic for this node. Should be implemented by subclasses to define specific computation logic.

        Parameters:
            tables (Dict[str, PhenexTable]): A dictionary mapping table names to PhenexTable objects.
