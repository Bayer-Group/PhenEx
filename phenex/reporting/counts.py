...
class InExCounts(Reporter):
    ...
    def execute(self, cohort: "Cohort") -> pd.DataFrame:
        self.cohort = cohort
        self.df_counts_inclusion = self.get_counts_for_phenotypes(self.cohort.inclusions, 'inclusion')
        self.df_counts_exclusion = self.get_counts_for_phenotypes(self.cohort.exclusions, 'exclusion')
        return pd.concat([self.df_counts_inclusion, self.df_counts_exclusion])
    
    def get_counts_for_phenotypes(self, phenotypes: List["Phenotype"], category: str = None):
        ds = []
