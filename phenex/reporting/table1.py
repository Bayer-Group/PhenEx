...
from phenex.util.logging import getLoo logging
...
class Table1(Reporter):
    ...
    def execute(self, cohort: "Cohort") -> pd.DataFrame:
        ...
        if self.df_booleans is not None and self.df_values is not None:
            ...
            self.df = self.df_booleans if self.df_booleans is not None else self.df_values
    ...
    def _get_boolean_characteristics(self):
        return [x for x in self.cohort.characteristics if type(x).__name__ not in ['MeasurementPhenotype', 'AgePhenotype']]
