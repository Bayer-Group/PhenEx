...
class StackableRegimen:
    def __init__(
        ...
        name: str = "sr",
        censored_phenotype: Optional[Any] = None,
    ) -> List[LogicPhenotype]:
        """
        ...
        Parameters:
            ...
            censored_phenotype: Optional phenotype identifying censored patients. Like "None", this is a standalone mutually exclusive category: censored patients are excluded from every stack/None bin and reported only in the "Censored" bin.
        ...
        self.name = name
        self.censored_phenotype = censored_phenotype
    ...
    def _generate_n_regimen_combinations(
        ...
        for size in range(1, n_regimens + 1):
            ...
            for combo in combinations(regimen_keys, size):
                ...
                # Censored patients are reported only in the standalone "Censored" bin
                if self.censored_phenotype is not None:
                    logic = logic & ~self.censored_phenotype
        ...
        none_logic = ~all_combined
        if self.censored_phenotype is not None:
            none_logic = none_logic & ~self.censored_phenotype
        none_phenotype = LogicPhenotype(name=f"{prefix}_NONE", expression=none_logic)
        none_phenotype.display_name = "None"
        results["none"] = [none_phenotype]

        # Add standalone "censored" category, mutually exclusive from all bins above
        if self.censored_phenotype is not None:
            self.censored_phenotype.display_name = "Censored"
            results["censored"] = [self.censored_phenotype]
