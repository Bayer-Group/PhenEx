...
class SimpleCohortWithExclusionAndStudyPeriodTestGenerator(SimpleCohortWithExclusionTestGenerator):
    """
...
class SimpleCohortWithExclusionPostIndexTestGenerator(SimpleCohortWithExclusionTestGenerator):
    """
...
class SimpleCohortWithInclusionAndExclusionTestGenerator(CohortTestGenerator):
    ...
    def define_mapped_tables(self):
        ...
        # create drug exposure table
        df_drug_exposure_entry = pd.DataFrame(df_allvalues[["PATID", "entry", "entry_date"]])
        df_drug_inclusion_exposure = pd.DataFrame(df_allvalues[["PATID", "inclusions", "inclusions_date"]])
        df_drug_exclusion_exposure = pd.DataFrame(df_allvalues[["PATID", "exclusions", "exclusions_date"]])
        df_drug_exposure_entry.columns = ["PATID", "PRODCODEID", "ISSUEDATE"]
        df_drug_inclusion_exposure.columns = ["PATID", "PRODCODEID", "ISSUEDATE"]
        df_drug_exclusion_exposure.columns = ["PATID", "PRODCODEID", "ISSUEDATE"]
        df_drug_exposure = pd.concat([df_drug_exposure_entry,df_drug_inclusion_exposure, df_drug_exclusion_exposure])
...
class SimpleCohortWithInclusionAndExclusionSeparateTablesTestGenerator(CohortTestGenerator):
    def define_cohort(self):
        entry = CodelistPhenotype(
            return_date="first",
            codelist=Codelist(["d1"]).resolve(use_code_type=False),
    ...
    def define_mapped_tables(self):
        ...
        df_inclusion = pd.DataFrame(df_allvalues[["PATID", "inclusions", "inclusions_date"]])
...
class SimpleCohortWithContinuousCoverageInclusionTestGenerator(SimpleCohortWithExclusionTestGenerator):
    def define_cohort(self):
        entry = CodelistPhenotype(
            return_date="first",
            codelist=Codelist(["d1"]).resolve(use_code_type=False),
