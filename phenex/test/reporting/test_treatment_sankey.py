...
def test_sankey_html_from_fixture():
    ...
    period1_links = [lk for lk in fixture_entry["links"] if lk["source"] in {2, 4, 5}]
    fzt_to_fzt = next(
        lk for lk in period1_links if lk["source"] == 2 and lk["target"] == 9
    )
    assert fzt_to_fzt["value"] == 7248

    fzt_to_htfzt = next(
        lk for lk in period1_links if lk["source"] == 2 and lk["target"] == 11
    )
