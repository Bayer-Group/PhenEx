...
class TestCoalescingMapper(CodeTable):
    ...
    DEFAULT_MAPPING = {
        "PERSON_ID": "PERSON_ID",
        "EVENT_DATE": ["START_DATE", "RECORDED_DATE", "CREATED_DATE"],  # Coalesce multiple date columns
        "CODE": "EVENT_CODE",
    }
...
class TestPersonCoalescingMapper(PhenexPersonTable):
    ...
    DEFAULT_MAPPING = {
        "PERSON_ID": "PERSON_ID",
        "DATE_OF_BIRTH": ["BIRTH_DATETIME", "BIRTH_DATE"],  # Prefer datetime, fallback to date
    }
...
def test_coalesce_first_column_present():
    ...
    print("Expected:"); print(df_expected)
    print("Got:"); print(result[["EVENT_DATE"]])

    # Should use START_DATE for all rows (cast to timestamp)
    pd.testing.assert_frame_equal(result[["EVENT_DATE"]], df_expected, check_dtype=False)
...
def test_single_column_mapping():
    ...
    print("Expected:"); print(df_expected)
    print("Got:"); print(result[["EVENT_DATE", "CODE"]])

    pd.testing.assert_frame_equal(result[["EVENT_DATE", "CODE"]], df_expected, check_dtype=False)
...
def test_person_table_coalescing():
    ...
    print("Expected:"); print(df_expected)
    print("Got:"); print(result[["DATE_OF_BIRTH"]])

    pd.testing.assert_frame_equal(result[["DATE_OF_BIRTH"]], df_expected, check_dtype=False)
...
def test_mixed_mapping():
    ...
    print("Expected:"); print(df_expected)
    print("Got:"); print(result[["PERSON_ID", "EVENT_DATE", "CODE"]])

    pd.testing.assert_frame_equal(result[["PERSON_ID", "EVENT_DATE", "CODE"]], df_expected, check_dtype=False)
...
def test_column_mapping_override():
    ...
    mapped_table = TestSingleColumnMapper(
        table, 
        column_mapping={"CODE": "CUSTOM_CODE"}
    )
