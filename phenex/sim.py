...
class DomainsMocker:
    ...
    def _get_visit_detail_ids_pool(self) -> np.ndarray:
        ...
        if self._visit_detail_ids_pool is None:
            # Poisson(lam=1.5) clipped at 8 means max n_patients * 8 visit details
            n_visit_details = int(self.n_patients * 9)
    ...
    def _get_visit_occurrence_ids_pool(self) -> np.ndarray:
        ...
        if self._visit_occurrence_ids_pool is None:
            # Poisson(lam=1.8) clipped at 10 means max n_patients * 10 visit occurrences
            n_visit_occurrences = int(self.n_patients * 11)
...
class DatabaseMocker:
    """
    DatabaseMocker creates a mock :class:`~phenex.core.Database` backed by in-memory
    DuckDB tables generated by :class:`DomainsMocker`. The resulting database object
    can be passed directly to :class:`~phenex.core.Cohort` and
    :class:`~phenex.core.study.Study`, making it easy to run end-to-end tests without
    a real database connection.

    Parameters:
        domains_dict: The domains dictionary describing which tables to mock. Defaults to ``OMOPDomains``.
        n_patients: Number of simulated patients. Defaults to 100.
        random_seed: Random seed for reproducible results. Defaults to 42.
    ...
    Example:
        ...
        db = DatabaseMocker(n_patients=200).get_database()
        ...
        cohort = Cohort(
            ...
            entry_criterion=CodelistPhenotype(
                name="af",
                domain="CONDITION_OCCURRENCE",
                codelist=Codelist([4119602]),
            ),
            database=db,
        )

        study = Study(name="test", path="./out", cohorts=[cohort])
        study.execute()
        ```
    """
    ...
    def __init__(
        self,
        domains_dict: DomainsDictionary = None,
        n_patients: int = 100,
        random_seed: int = 42,
    ):
        from phenex.mappers import OMOPDomains

        self.domains_dict = domains_dict or OMOPDomains
        self.n_patients = n_patients
        self.random_seed = random_seed

    def get_database(self) -> "Database":
        """Create and return a mock :class:`~phenex.core.Database`.

        The database uses an in-memory DuckDB connection with the simulated
        tables registered on it and the ``domains_dict`` as its mapper, so
        cohort execution retrieves tables exactly as it would against a real
        database.

        Returns:
            A :class:`~phenex.core.Database` ready to be passed to
            :class:`~phenex.core.Cohort`.
        """
        from phenex.core.database import Database
        from phenex.ibis_connect import DuckDBConnector

        mocker = DomainsMocker(
            domains_dict=self.domains_dict,
        ...
        duckdb_con = connector.source_connection.con
        for table_name, ibis_table in source_tables.items():
            df = ibis_table.to_pandas()
            duckdb_con.register("_tmp", df)
            duckdb_con.execute(
                f"CREATE OR REPLACE TABLE {table_name} AS SELECT * FROM _tmp"
            )
            duckdb_con.unregister("_tmp")

        return Database(connector=connector, mapper=self.domains_dict)
