...
class PhenexPythonToRConverter:
    ...
    def _setup_conversion_rules(self) -> List[ConversionRule]:
        ...
        return [
            ...
            ConversionRule(
                r"\[([^\]]*)\]",
                r"list(\1)",
                "Convert Python lists to R lists"
            ),
            ...
            ConversionRule(
                r"\b(\d+)\b",
                r"\1L",
                "Add L suffix to integers for R"
            ),
    ...
    def convert_python_file(self, input_path: str, output_path: Optional[str] = None) -> str:
    ...
    def convert_notebook(self, input_path: str, output_path: Optional[str] = None) -> str:
        """Convert a Jupyter notebook to R Markdown or Quarto"""
        with open(input_path, 'r', encoding='utf-8') as f:
            notebook = json.load(f)
        
        # Determine output format
        if output_path and (output_path.endswith('.qmd') or output_path.endswith('.Rmd')):
        ...
        for i, cell in enumerate(notebook.get('cells', [])):
            if cell['cell_type'] == 'code':
                ...
                if source:
                    ...
                    python_code = ''.join(source) if isinstance(source, list) else source
                ...
                if source:
                    ...
                    markdown_text = ''.join(source) if isinstance(source, list) else source
...
Examples:
    ...
    parser.add_argument('input_file', help='Input Python file (.py) or Jupyter notebook (.ipynb) to convert')
    parser.add_argument('output_file', nargs='?', help='Output R file (.R), R Markdown (.Rmd), or Quarto (.qmd) file (optional)')
    parser.add_argument(
        '--interactive', 
        action='store_true', 
    ...
    parser.add_argument(
        '--preview', 
        action='store_true', 
        help='Preview conversion without writing file'
    )
    parser.add_argument(
        '--format',
        choices=['r', 'rmd', 'qmd'],
    ...
    try:
        if args.preview:
            ...
            if converter.manual_review_items:
                print(f"🔍 {len(converter.manual_review_items)} items need manual review")
                if output_path.suffix.lower() in ['.qmd', '.rmd']:
                    print("   Check the 'Migration Notes' section at the end of the document")
                else:
            ...
            if args.interactive:
                print("\n📋 NEXT STEPS:")
                if output_path.suffix.lower() in ['.qmd', '.rmd']:
                    print("1. Open the generated file in RStudio or a Quarto-compatible editor")
                    print("2. Review the 'Migration Notes' section")
                    print("3. Address all manual review items")
                    print("4. Run code chunks interactively or render the entire document")
